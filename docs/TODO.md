...
- [ ] **Fix Nix flake for seamless dev onboarding**: The current `flake.nix` only provides `node` and `pnpm` but doesn't auto-run `pnpm install` or handle `direnv` integration. Improve so entering the project directory is zero-friction:
